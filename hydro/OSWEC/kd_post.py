...
xR = np.linspace(-2*int(int_lam),0,int(int_lam // 2))
#print(xR)
xD = np.linspace(0,2*int(int_lam),int(int_lam // 2))
xt = np.linspace(-2*int(int_lam),2*int(int_lam),int(int_lam))
y0 = np.zeros_like(xt)
...
zinc_up = inc_el.applymap(lambda x: complex(x.replace(' ', ''))).iloc[int(int_lam // 2)-1,:int(int_lam // 2)]
kd_up = kd1.applymap(lambda x: complex(x.replace(' ', ''))).iloc[int(int_lam // 2)-1,:int(int_lam // 2)]
...
######################################## FOR DOWNSTREAM ###########################################
zinc_down = inc_el.applymap(lambda x: complex(x.replace(' ', ''))).iloc[int(int_lam // 2)-1,int(int_lam // 2):]
kd_down = kd1.applymap(lambda x: complex(x.replace(' ', ''))).iloc[int(int_lam // 2)-1,int(int_lam // 2):]

def peaks_down(series, lambda_wave):
    peaks = []
    for i in range(1, len(series) - 1):  
        if np.angle(series[i] - series[i - 1]) > 0 and np.angle(series[i] - series[i + 1]) > 0:
            if xD[i] < 1*lambda_wave:  # Limiting to x < lambda_wave
...
plt.legend()
plt.savefig(f'elev_disturb/050_kdcomp.pdf')
plt.show()
