...
def hydro(array,B,depth,w,char_dim,farm,controls,point_absorber,oscillating_surge,budal_limit):
    ...
    if controls == True:
        RAO_controlled, CWR, B_pto, power = PTO.RAO(diff_prob,diff_result,dataset,array,w,farm,char_dim,budal_limit,point_absorber,oscillating_surge)
        RAO_vals = RAO_controlled
        print('controlled rao',RAO_vals)
    return diff_result,rad_result,RAO_vals,lam, CWR, B_pto,power
...
def elevation(res,lam,diff_result,rad_result,RAO_vals,farm,rad,controls,N,attenuator,rel_dim):
    ...
    return total, rad_dif, incoming_fse, x1, x2, nx, y1, y2, ny