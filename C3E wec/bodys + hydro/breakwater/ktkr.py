...
def hydrodynamic_calculations(w, B, depth, wi, th, h, x, y, z, nw, nt, nh):
    ...
    total = diffraction + incoming_fse                          # total wave el

    return incoming_fse, total, diffraction

def post_process_wave_data(xR, xD, diffraction, incoming_fse, total):
    # extracting the center line of wave elevation for computation
    z_up = np.real(-1*diffraction[:75,74])            # just the rad + dif upstream
    z_down = np.real(-1*total[75:,74])             # total wave elevation
    zinc_up = np.real(-1*incoming_fse[:75,74])     # incident wave elevation
    zinc_down = np.real(-1*incoming_fse[75:,74])   # incident wave elevation
...
for freq in w:
    ...
    incoming_fse, total, diffraction = hydrodynamic_calculations(freq, B, depth, wi, th, h, x, y, z, nw, nt, nh)
    # Calculate Kt and Kr for the current frequency
    Kt, Kr = post_process_wave_data(xR, xD, diffraction, incoming_fse, total)
