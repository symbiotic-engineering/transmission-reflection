...
def hydrodynamic_calculations(w, B, depth, wi, th, h, x, y, z, nw, nt, nh):
    ...
    # defining mesh
    body = cpt.FloatingBody(cpt.meshes.predefined.rectangles.mesh_parallelepiped(size=(wi, th, h), resolution=(nw, nt, nh), 
                                                                                 center=(x, y, z), name='rect'))
...
def post_process_wave_data(xR, xD, upeffect, incoming_fse, total):
    ...
    z_up = np.real(upeffect[:75,74])            # just the rad + dif upstream
    z_down = np.real(total[75:,74])             # total wave elevation
    zinc_up = np.real(incoming_fse[:75,74])     # incident wave elevation
    zinc_down = np.real(incoming_fse[75:,74])   # incident wave elevation
...
for freq in w:
    ...
    incoming_fse, total, upeffect = hydrodynamic_calculations(freq, B, depth, wi, th, h, x, y, z, nw, nt, nh)
