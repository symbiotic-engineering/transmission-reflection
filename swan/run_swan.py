def generate_swan_input(KR, KT, d, x, ya, yb, H, T, xgrid, ygrid, mxc, myc, attenuator):
    ...
    for i in range(len(x)):
        ...
        if EB > 1:
            ...
            KR[i] = 0
        if attenuator == True:
            length = 29             # y-length of attenuator
            #d = 15
            #xe = [xi + d for xi in x]
            # i == 1 for 3bod staggered, i < 3 for all other cases
            #line = f"OBSTACLE TRANS {KT[i]} REFL {KR[i]} RDIFF 1 LINE {x[i]} {ya if i == 1 else yb} {xe[i]} {ya+length if i == 1 else yb-length}"
            line = f"OBSTACLE TRANS {KT[i]} REFL {KR[i]} RDIFF 1 LINE {x[i]} {ya if i < 3 else yb} {xe[i]} {ya+length if i < 3 else yb-length}"
        else:
