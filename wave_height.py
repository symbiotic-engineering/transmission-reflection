...
z_up = el.applymap(lambda x: complex(x.replace(' ', ''))).iloc[int(int_lam // 2)-1,:int(int_lam // 2)]
zinc_up = inc_el.applymap(lambda x: complex(x.replace(' ', ''))).iloc[int(int_lam // 2)-1,:int(int_lam // 2)]
z_down = el.applymap(lambda x: complex(x.replace(' ', ''))).iloc[int(int_lam // 2)-1,int(int_lam // 2):]
...
plt.savefig(f'elevation/050_elev.pdf')
plt.show()