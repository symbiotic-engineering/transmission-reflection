def lpf(w,res,xtrans,ytrans,depth,froude_number,farm):
    ...
    if farm==False:
        ...
        excitationForce_SWELL = np.array([16.7575566115100,15.2911835464861,17.2644688499461])    #/(froude_number**3)
    ...
    F_ex = (FK + diff_force)*0.05       # multipled by 0.05 bc SWELL incident wave height
    print('capy ex force',abs(F_ex))
    ...
    diffraction = solver.compute_free_surface_elevation(grid, diff_result)*0.05 # multipled by 0.05 bc SWELL incident wave height
    multiplications = []
    if farm == False:
        for i in range(1):
            mult_result = solver.compute_free_surface_elevation(grid, rad_result[i]) * experimental_RAO[i]
